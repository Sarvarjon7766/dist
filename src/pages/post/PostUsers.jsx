...
const PostUsers = () => {
		...
		try {
			...
			if (usersRes.data.success && deptRes.data.success) {
				const processedUsers = usersRes.data.users.map(user => {
					// Kechikkanlikni barcha xodimlar uchun hisoblash
					const isUserLate = user.firstCheckInTime && isLate(user.firstCheckInTime)
					return {
						...user,
						...
						isLate: isUserLate,
						lateMinutes: isUserLate ? calculateLateMinutes(user.lastCheckInTime) : 0,
						// Asl UTC vaqtlarni ham saqlab qo'yamiz
	...
	const calculateStats = () => {
		...
		return {
			...
			kechikkan: {
				...
				// Barcha xodimlarga nisbatan foiz
	...
	return (
		<div className="min-h-screen bg-gray-50 p-4">
			...
			<div className="bg-white rounded-xl shadow p-4 mb-6 overflow-hidden">
				<div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between flex-wrap">
					{/* Title */}
					<h1 className="text-xl sm:text-2xl font-bold text-indigo-600 flex items-center flex-shrink-0">
					...
					<div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto flex-wrap">
						...
						{/* Department Filter */}
						<div className="flex items-center gap-2 bg-gray-100 px-3 py-2 rounded-lg w-full sm:w-48">
						...
						<div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
							<button
								...
								className="bg-gray-200 hover:bg-gray-300 text-gray-800 px-3 py-2 rounded-lg flex items-center justify-center gap-2 text-sm transition-colors"
								title="Filterlarni tozalash"
							>
								<FiX size={14} />
								<span className="whitespace-nowrap">Tozalash</span>
							</button>
							...
							<button
								...
								disabled={refreshing}
								className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg flex items-center justify-center gap-2 text-sm transition-colors"
								title="Yangilash"
							>
								{refreshing ? (
			...
			<div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
				...
				<div
					className="bg-white rounded-lg shadow p-4 flex items-center hover:shadow-md transition-shadow cursor-pointer"
					onClick={() => {
						// Kechikkanlar statistik kartasini bosganda, faqat kechikkan xodimlarni ko'rsatish
						setSelectedStatus('all')
						setSearchTerm('')
						setSelectedDepartment('all')
					}}
                  ...
                  `}>
										...
										<td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
											<div className="flex items-center gap-1">
												...
												{/* Agar lastCheckInTime yo'q bo'lsa yoki ikkalasi ham to'lgan bo'lsa */}
												{user.isEdit && (!user.lastCheckInTime || (user.lastCheckInTime && user.lastCheckOutTime)) && (
													<button
										...
										<td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
											<div className="flex items-center gap-1">
												...
												{/* Faqat kirish bor, chiqish yo'q holatda */}
												{user.isEdit && user.lastCheckInTime && !user.lastCheckOutTime && (
													<button
										...
										<td className="px-4 py-4 text-sm text-gray-900 whitespace-normal break-words max-w-xs">
											<div className="flex items-center gap-1">
												...
												{/* Faqat chiqish hali qo'shilmagan bo‘lsa */}
												{user.isEdit && user.lastCheckInTime && !user.lastCheckOutTime && (
													<button
